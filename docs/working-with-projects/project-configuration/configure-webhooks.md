...
### Discord Endpoint Payload Example

When using a Discord endpoint, Harbor sends an embed-formatted message:
  ...
  "embeds": [
    {
      ...
      "description": "Event Type: PUSH_ARTIFACT\nOperator: admin\nTimestamp: 2023-04-03T06:04:46Z",
      "fields": [
        {
          "name": "Event Data",
          "value": "Repository: harbor/alpine\nDigest: sha256:954b378c375d852eb3c63ab88978f640b4348b01c1b3456a024a81536dafbbf4",
          "inline": false
        }
      ],
      "color": 16711680
...
- **DISCORD**: Discord webhook for embed-formatted messages
- **MATRIX**: Matrix room messaging with markdown support
- **EMAIL**: SMTP email notifications using Harbor's email configuration
- **AMQP**: Message queue publishing to AMQP brokers
- **TEAMS**: Microsoft Teams webhook connector
...
### Discord Endpoint Configuration

For Discord endpoints, you should create a webhook in your Discord server following the [Discord webhook guide](https://support.discord.com/hc/en-us/articles/228383668-Intro-to-Webhooks). The webhook URL will be used as the endpoint address.

When you select the Discord notify type, Harbor will format the notification as a Discord embed with rich formatting including event type, timestamp, operator, and event data.
...
1. Select the notify type from the available options: `HTTP`, `SLACK`, `DISCORD`, `MATRIX`, `EMAIL`, `AMQP`, `TEAMS`, or `TELEGRAM`.
