...
### Matrix Endpoint Payload Example

For Matrix, messages use markdown formatting:
...
- **Data**: Repository harbor/alpine pushed with digest sha256:954b378c375d852eb3c63ab88978f640b4348b01c1b3456a024a81536dafbbf4
```
...
### Matrix Endpoint Configuration

For Matrix endpoints, you can use a Matrix webhook URL or configure a bot to receive messages. The endpoint should accept JSON payloads with Matrix message format.

When you select the Matrix notify type, Harbor will format the notification as a Matrix room message with markdown formatting.
...
1. Select the notify type from the available options: `HTTP`, `SLACK`, or `MATRIX`.
